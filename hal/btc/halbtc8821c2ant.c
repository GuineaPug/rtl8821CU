...
u32 glcoex_ver_date_8821c_2ant = 20180516;
u32 glcoex_ver_8821c_2ant = 0x2e;
				    ...
				    u8 pos_type)
{
	...
	if (ctrl_type == BT_8821C_2ANT_CTRL_BY_FW)
		return;
...
u8 halbtc8821c2ant_action_algorithm(struct btc_coexist *btc)
{
	...
	u8 alg = BT_8821C_2ANT_COEX_UNDEFINED;
	...
	if (num_of_diff_profile == 0) {
		if (bt_link_info->acl_busy) {
			...
			BTC_TRACE(trace_buf);
			alg = BT_8821C_2ANT_COEX_NOPROFILEBUSY;
		}
	} else if (bt_link_info->a2dp_exist && coex_sta->is_bt_a2dp_sink) {
		BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
			    "[BTCoex], A2DP Sink\n");
		BTC_TRACE(trace_buf);
		alg = BT_8821C_2ANT_COEX_A2DPSINK;
	} else if (num_of_diff_profile == 1) {
		if (bt_link_info->sco_exist) {
			BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
				    "[BTCoex], SCO only\n");
			BTC_TRACE(trace_buf);
			alg = BT_8821C_2ANT_COEX_SCO;
		} else {
			if (bt_link_info->hid_exist) {
				BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
					    "[BTCoex], HID only\n");
				BTC_TRACE(trace_buf);
				alg = BT_8821C_2ANT_COEX_HID;
			} else if (bt_link_info->a2dp_exist) {
				BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
					    "[BTCoex], A2DP only\n");
				BTC_TRACE(trace_buf);
				alg = BT_8821C_2ANT_COEX_A2DP;
			} else if (bt_link_info->pan_exist) {
				if (bt_hs_on) {
					BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
						    "[BTCoex], PAN(HS) only\n");
					BTC_TRACE(trace_buf);
					alg = BT_8821C_2ANT_COEX_PANHS;
				} else {
					BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
						    "[BTCoex], PAN(EDR) only\n");
					BTC_TRACE(trace_buf);
					alg = BT_8821C_2ANT_COEX_PAN;
				}
			}
		}
			...
			if (bt_link_info->hid_exist &&
			    bt_link_info->a2dp_exist) {
				{
					BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
						    "[BTCoex], HID + A2DP\n");
					BTC_TRACE(trace_buf);
					alg = BT_8821C_2ANT_COEX_HID_A2DP;
				}
				...
				if (bt_hs_on) {
					...
					BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
						    "[BTCoex], HID + PAN(EDR)\n");
					BTC_TRACE(trace_buf);
					alg = BT_8821C_2ANT_COEX_PAN_HID;
				}
			} else if (bt_link_info->pan_exist &&
				   bt_link_info->a2dp_exist) {
				if (bt_hs_on) {
					BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
						    "[BTCoex], A2DP + PAN(HS)\n");
					BTC_TRACE(trace_buf);
					alg = BT_8821C_2ANT_COEX_A2DP_PANHS;
				} else {
					BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
						    "[BTCoex], A2DP + PAN(EDR)\n");
					BTC_TRACE(trace_buf);
					alg = BT_8821C_2ANT_COEX_PAN_A2DP;
				}
			}
		}
		...
		if (bt_link_info->sco_exist) {
			...
			if (bt_link_info->hid_exist &&
				...
				if (bt_hs_on) {
					...
					BTC_TRACE(trace_buf);
					alg = BT_8821C_2ANT_COEX_HID_A2DP_PAN;
				} else {
					BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
