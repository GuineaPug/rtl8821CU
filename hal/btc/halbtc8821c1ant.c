...
u32 glcoex_ver_date_8821c_1ant = 20180516;
u32 glcoex_ver_8821c_1ant = 0x2e;
				    ...
				    u8 pos_type)
{
	...
	if (ctrl_type == BT_8821C_1ANT_CTRL_BY_FW)
		return;
	...
	case BT_8821C_1ANT_CTRL_BY_BBSW:
		...
		btc->btc_write_1byte_bitmask(btc, 0xcb7, 0x30, regval_0xcb7);
		break;
	case BT_8821C_1ANT_CTRL_BY_PTA:
		 ...
		 */
		regval_0xcb7 = (!switch_polarity_inverse ? 0x2 : 0x1);
		btc->btc_write_1byte_bitmask(btc, 0xcb7, 0x30, regval_0xcb7);
		break;
	...
	case BT_8821C_1ANT_CTRL_BY_MAC:
		...
		btc->btc_write_1byte_bitmask(btc, 0x64, 0x1, regval_0x64);
		break;
...
u8 halbtc8821c1ant_action_algorithm(struct btc_coexist *btc)
{
	...
	u8 alg = BT_8821C_1ANT_COEX_UNDEFINED;
	...
	if (num_of_diff_profile == 1) {
		if (bt_link_info->sco_exist) {
			...
			BTC_TRACE(trace_buf);
			alg = BT_8821C_1ANT_COEX_SCO;
		} else {
			if (bt_link_info->hid_exist) {
				BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
					    "[BTCoex], BT Profile = HID only\n");
				BTC_TRACE(trace_buf);
				alg = BT_8821C_1ANT_COEX_HID;
			} else if (bt_link_info->a2dp_exist) {
				BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
					    "[BTCoex], BT Profile = A2DP only\n");
				BTC_TRACE(trace_buf);
				alg = BT_8821C_1ANT_COEX_A2DP;
			} else if (bt_link_info->pan_exist) {
				if (bt_hs_on) {
					BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
						    "[BTCoex], BT Profile = PAN(HS) only\n");
					BTC_TRACE(trace_buf);
					alg = BT_8821C_1ANT_COEX_PANHS;
				} else {
					BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
						    "[BTCoex], BT Profile = PAN(EDR) only\n");
					BTC_TRACE(trace_buf);
					alg = BT_8821C_1ANT_COEX_PAN;
				}
			}
		}
		...
		if (bt_link_info->sco_exist) {
			if (bt_link_info->hid_exist) {
				...
				if (bt_hs_on) {
					...
					BTC_TRACE(trace_buf);
					alg = BT_8821C_1ANT_COEX_PAN_HID;
				}
			}
		} else {
			if (bt_link_info->hid_exist &&
				...
				BTC_TRACE(trace_buf);
				alg = BT_8821C_1ANT_COEX_HID_A2DP;
				...
				if (bt_hs_on) {
					...
					BTC_TRACE(trace_buf);
					alg = BT_8821C_1ANT_COEX_A2DP_PANHS;
				} else {
					BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
						    "[BTCoex], BT Profile = A2DP + PAN(EDR)\n");
					BTC_TRACE(trace_buf);
					alg = BT_8821C_1ANT_COEX_PAN_A2DP;
				}
			}
		}
		...
		if (bt_link_info->sco_exist) {
			...
			if (bt_link_info->hid_exist &&
				...
				if (bt_hs_on) {
					...
					BTC_TRACE(trace_buf);
					alg = BT_8821C_1ANT_COEX_HID_A2DP_PAN;
				}
			}
		}
	} else if (num_of_diff_profile >= 3) {
	...
	if (bt_link_info->sco_exist) {
		...
		if (wifi_cckdeadlock_ap &&
			...
			if (coex_sta->hid_busy_num < 2)
				...
				halbtc8821c1ant_table(btc, NM_EXCU, 6);
			halbtc8821c1ant_tdma(btc, NM_EXCU, TRUE, 18);
...
void halbtc8821c1ant_action_bt_acl_busy(struct btc_coexist *btc)
{
	...
	if (bt_link_info->a2dp_exist && coex_sta->is_bt_a2dp_sink) {
		...
		if (coex_sta->wl_noisy_level == 0 && wifi_cckdeadlock_ap) {
			...
			if (coex_sta->connect_ap_period_cnt > 0 || !wifi_busy)
				halbtc8821c1ant_tdma(btc, NM_EXCU, TRUE, 26);
			else if (coex_sta->hid_pair_cnt > 1)
				halbtc8821c1ant_tdma(btc, NM_EXCU, TRUE, 24);
			else
...
void halbtc8821c1ant_action_wifi_connected(struct btc_coexist *btc)
{
	...
	switch (algorithm) {
	case BT_8821C_1ANT_COEX_SCO:
		BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
			    "[BTCoex], Action algorithm = SCO.\n");
		BTC_TRACE(trace_buf);
		break;
	case BT_8821C_1ANT_COEX_HID:
		BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
			    "[BTCoex], Action algorithm = HID.\n");
		BTC_TRACE(trace_buf);
		break;
	case BT_8821C_1ANT_COEX_A2DP:
		BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
			    "[BTCoex], Action algorithm = A2DP.\n");
		BTC_TRACE(trace_buf);
		break;
	case BT_8821C_1ANT_COEX_A2DP_PANHS:
		BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
			    "[BTCoex], Action algorithm = A2DP+PAN(HS).\n");
		BTC_TRACE(trace_buf);
		break;
	case BT_8821C_1ANT_COEX_PAN:
		BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
			    "[BTCoex], Action algorithm = PAN(EDR).\n");
		BTC_TRACE(trace_buf);
		break;
	case BT_8821C_1ANT_COEX_PANHS:
		BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
			    "[BTCoex], Action algorithm = HS mode.\n");
		BTC_TRACE(trace_buf);
		break;
	case BT_8821C_1ANT_COEX_PAN_A2DP:
		BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
			    "[BTCoex], Action algorithm = PAN+A2DP.\n");
		BTC_TRACE(trace_buf);
		break;
	case BT_8821C_1ANT_COEX_PAN_HID:
		BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
			    "[BTCoex], Action algorithm = PAN(EDR)+HID.\n");
		BTC_TRACE(trace_buf);
		break;
	case BT_8821C_1ANT_COEX_HID_A2DP_PAN:
		BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
			    "[BTCoex], Action algorithm = HID+A2DP+PAN.\n");
		BTC_TRACE(trace_buf);
		break;
	case BT_8821C_1ANT_COEX_HID_A2DP:
		BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
